...
export default function BasicColors1999Box({
  ...
  const toggleBasicColorsButtonText =
    clickBasic === true
      ? 'Hide "Basic 1999" Colors'
      : 'Get "Basic 1999" Colors';

  return (
    <div className="basic-paintbox">
      <button className="get-basic-colors" onClick={handleClickGetBasicColors}>
        {toggleBasicColorsButtonText}
      </button>
      <div className="grid-box">{showMyBasicColors}</div>
    </div>
  );
}
