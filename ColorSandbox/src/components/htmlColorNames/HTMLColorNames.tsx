...
export default function HTMLColors({
  ...
  const toggleHTMLColorsButtonText =
    clickHTMLColors === true
      ? "Hide Named HTML Colors"
      : "Get HTML Named Colors";

  return (
    <div className="html-paintbox">
      <button className="get-html-colors" onClick={handleClickGetHTMLColors}>
        {toggleHTMLColorsButtonText}
      </button>
      <div className="grid-box">{showMyHTMLColors}</div>
    </div>
  );
}
